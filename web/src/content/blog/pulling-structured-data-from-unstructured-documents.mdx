...
This post walks through that toolbox and, more importantly, *when to reach for each piece*. The code is fresh and generic, built on a couple of invented sample documents; nothing here is tied to a particular product or dataset.

## The toolbox

There's a temptation, especially now, to throw every document at an LLM and ask for JSON. Sometimes that's the right call. Often it's the expensive, slow, and least reliable option for fields a five-line regex would nail. It helps to know the full menu:
...
- **LLM extraction** — the most flexible option. Hand the model some text and a schema, get structured output back. It handles messy, varied, semantically-loaded fields that defeat rules. The costs are real, though: latency, money, and a tendency to invent plausible values that aren't in the document. That last one is the whole reason for grounding, which we'll get to.

The skill isn't picking one. It's routing each *field* to the cheapest technique that handles it well, and only escalating when you have to.
...
With the text in hand, knock out the easy fields with deterministic rules. Emails, phone numbers, and amounts have regular enough surface forms that a regex is both faster and more trustworthy than any model — and when it's wrong, you can see exactly why.
...
Some fields resist all of the above. "What is this candidate's most senior role?" or "Summarise the dietary tags for this recipe" require reading and judgement, not pattern matching. This is where an LLM earns its cost — but you have to constrain it and verify it.

Two things make LLM extraction trustworthy:

1. **Force the shape.** Give the model a JSON schema and require the output to conform to it, so you get a key for every field and nothing extra. Modern APIs support this directly — see OpenAI's [Structured Outputs](https://platform.openai.com/docs/guides/structured-outputs), which guarantees the response matches a supplied JSON Schema, and Anthropic's [tool use](https://docs.anthropic.com/en/docs/build-with-claude/tool-use), where a tool's `input_schema` plays the same role.
...
The grounding step is deliberately strict — exact-substring is a starting point, and in practice you'll relax it (normalise whitespace, allow fuzzy matches for reformatted dates or amounts). But the principle holds: *an extracted value you can't trace back to the document is a claim, not a fact.* Treat it accordingly.
