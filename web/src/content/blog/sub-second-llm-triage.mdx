...
description: "I benchmarked a tiered LLM safety gate with 800 real API calls: a fast model on the hot path, a larger one for the cases that need reasoning, plus circuit breakers and a caution-biased fallback. The measurements upended two of my assumptions, starting with a 'tiny' model that turned out to be the slowest thing in the system."
...
I finally stopped drawing that curve from a synthetic harness and measured it: 800 real calls to the OpenAI API from my machine in Brazil, through the same [Mastra](https://mastra.ai/) agent stack and structured-output path the clinic assistant uses, one patient message per call. API latency drifts with provider load and your region, so treat the absolute numbers as a snapshot. The shape held, and it corrected three things I had assumed.

<Image src={latencyCdf} alt="CDF of per-request latency on a log scale: the prior-generation gpt-5-nano detector sits far to the right (p50 ~5.6s, p99 ~24s); the current gpt-5.4-nano fast tier and gpt-5.4-mini large tier cluster near 1 to 2.6s; the cascade tracks the fast tier for the bulk then steps right on the fraction it escalates." />

The first correction is the one I least expected. The *tiny* model I had shipped as the fast tier, `gpt-5-nano`, was the slowest thing in the system: the dashed curve out on the right, a 5.6-second median and a p99 of 23. It is a [reasoning model](https://artificialanalysis.ai/models/comparisons/gpt-5-nano-vs-gpt-4-1-nano), and at its default effort it burned hundreds of hidden reasoning tokens to answer a yes/no. A small model is not automatically a fast one. The current-generation `gpt-5.4-nano` answers the same question in about a second, so the fix was a one-line model bump, but I would not have gone looking without a tail to explain.

The second: once both tiers run on models that are actually fast, the latency gap between them is small. The fast tier (`gpt-5.4-nano`) runs a 1.1-second median; the large tier (`gpt-5.4-mini`) runs 1.6. Where the fast tier pulls ahead is the tail (p99 of 3.0 s against 6.0 s) and, as the next section shows, the cost. Raw median speed is not where it wins.

The third correction is about the word in the title. Nobody here clears 500 ms, and the fast tier does not even clear one second at the median. From this setup, with a 700-token system prompt and structured JSON output, sub-second is a target you engineer toward (a leaner prompt, a region closer to the provider, dropping structured output for a raw call), not a number these models hand you. The budget is the goal; the tail is what you design against.
...
The durable lessons are the ones that travel across implementations. Treat the latency budget as a distribution and design for the tail. Check that your "fast" model is actually fast, because a small reasoning model can be the slowest thing you run. Size the model to the job and escalate only when the cheap tier is unsure, since a cascade rescues the dangerous case only when the fast tier is honestly uncertain about it. Wrap the hot path in a breaker so a flaky upstream fails fast instead of cascading. Bias the fallback toward the safe error. And measure every tier, because a cascade is only as good as your ability to see when it stops paying off.
