...
I built this for an agent library that needed to survive provider outages and pricing changes without rewrites. Because every chatter answers to the same `answer(prompt)` contract, the surrounding code — retry logic, conversation summarization when history overflows the context window, the agent loop itself — is written exactly once and runs against all of them. When one provider rate-limits or degrades, falling back to another is swapping the class behind the interface, not rewriting the caller. (The fallback machinery itself is a separate concern; I wrote about graceful provider degradation and circuit breakers in an [earlier post](/blog/sub-second-llm-triage).)
