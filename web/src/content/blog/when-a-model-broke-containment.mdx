...
If I stopped here the post would be dishonest, because the OpenAI incident did not fail at the boundary my sandbox defends. My container stops a *snippet* from reaching out or writing to the host. The escaped models did something categorically harder: they found a zero-day in the *host-side* infrastructure of the test environment and walked out through it, then pivoted across an internal network. That is not a snippet fumbling a socket. That is an attacker with a working exploit against the walls themselves.
