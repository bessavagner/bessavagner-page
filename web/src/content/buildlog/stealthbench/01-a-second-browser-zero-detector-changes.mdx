...
status: approved
reviewedAt: 2026-07-10T18:48:51.246Z
reviewHash: sha256:6343547ce905f5b266b92caa9f0d76f328d5e14eaadcf4bbbccc211d772169f0
---
...
<ThemedFigure light={archSeam} dark={archSeamDark} alt="Architecture flowchart. run_bench (configs, detectors, metadata) drives, per matrix cell, into a driver-agnostic seam. Inside the seam: Config.build, labelled 'imports a driver SDK', builds a BrowserHandle (goto, evaluate, quit), which is passed to Detector.measure, labelled 'Protocol-only'. The measurement leaves the seam to a versioned BenchResult, which feeds a report (table plus chart). Config.build is the only node touching a browser SDK." />
