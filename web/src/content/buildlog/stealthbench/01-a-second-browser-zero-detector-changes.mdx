...
status: approved
reviewedAt: 2026-07-10T18:37:06.577Z
reviewHash: sha256:bfa8549ae06a844604bdb3f4eca0fa234f5352bedc0d9e6779ef7ef3501a53b2
---
...
<Image src={archSeam} alt="Architecture flowchart. run_bench (configs, detectors, metadata) drives, per matrix cell, into a driver-agnostic seam. Inside the seam: Config.build, labelled 'imports a driver SDK', builds a BrowserHandle (goto, evaluate, quit), which is passed to Detector.measure, labelled 'Protocol-only'. The measurement leaves the seam to a versioned BenchResult, which feeds a report (table plus chart). Config.build is the only node touching a browser SDK." class="rounded-2xl border border-primary/20 my-8 w-full h-auto" />
