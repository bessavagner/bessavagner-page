...
Here's the whole machine I'm building toward — and, honestly, how little of it exists yet. This update is the green half on top; everything dashed is the next plan.

<Image src={architecture} alt="ReplayGate's full architecture as two stages. The shipped 'record' half (top): an AI agent runs behind a channel adapter (direct shipped, WhatsApp on the roadmap); a RecordingLLMClient wraps the LLMClient protocol and a ToolRecorder wraps the tool registry, both writing to a fixture directory (conversation, llm, tool, spans, meta) and a DuckDB span store using OpenTelemetry gen_ai conventions; the span-wiring edge is dashed because it's deferred. The roadmap 'replay → diff → assert → gate' half (bottom): the fixture feeds a Replayer that runs it against a new model with tools mocked, a ConversationDiff compares baseline versus current path, cross-turn invariants like user_confirmed_before assert 'never books before confirm', and a replaygate regress command gates CI on pass or fail." />
