...
reviewHash: sha256:320d58566ee19da74575872c34955cda57c6b9f19007ce3b63c207746c6afaba
---

import { Image } from 'astro:assets';
import architecture from '../../../assets/buildlog/replaygate/architecture.svg';
...
<Image src={architecture} alt="Diagram. ReplayGate is a harness (a labelled box) that brackets an external agent under test, shown in amber at the top and marked 'your LLM agent, not part of ReplayGate; ships a BookingAgent example to demo against.' Shipped record-half components inside the box are green: a CLI invokes a channel adapter (direct shipped, WhatsApp on the roadmap) that feeds user turns into the agent; the agent's LLM calls and tool calls are intercepted by RecordingLLMClient and ToolRecorder, which wrap the agent's own LLM client and tools and write to a fixture directory (conversation, llm, tool, spans, meta) plus a DuckDB span store using OpenTelemetry gen_ai conventions; the span-wiring edge is dashed because it is deferred. Roadmap components are dashed grey: the fixture feeds a Replayer that re-runs it against a new model with tools mocked, a ConversationDiff compares baseline versus current path, cross-turn invariants such as user_confirmed_before assert 'never books before confirm', and a replaygate regress command gates CI on pass or fail." class="rounded-xl border border-base-content/15" />
