...
REFINE is the next legal stage after SCOPE, so the sequencing is fine. What the CLI refuses is the substance: you cannot enter REFINE until the artifact SCOPE is supposed to produce actually exists and has been recorded. The gate is not a sentence in a prompt hoping the agent reads it. It is a precondition the CLI evaluates against the state file, and a failed precondition is a non-zero exit and no state change. A [machine-readable plan the agent executes task by task](/blog/machine-readable-plan-format/) can be as detailed as you like, but the plan is advice; the state file is the law.
